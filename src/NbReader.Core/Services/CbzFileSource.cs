...
/// <summary>
/// CBZ 文件源：将 ZIP 压缩包中的图片条目作为漫画页。
/// </summary>
public class CbzFileSource : IFileSource
{
    ...
    /// <summary>
    /// 从 CBZ（ZIP）文件创建文件源。所有图片条目在构造时解压到内存。
    /// </summary>
    /// <param name="filePath">CBZ 文件路径。</param>
    /// <exception cref="FileNotFoundException">当文件不存在时抛出。</exception>
    /// <exception cref="InvalidOperationException">当压缩包损坏或格式无效时抛出。</exception>
    public CbzFileSource(string filePath)
    {
