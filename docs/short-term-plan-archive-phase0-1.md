# NbReader 短期计划（Phase 0 → Phase 1）✅ 基本完成

> 本计划覆盖从项目初始化到 MVP 本地阅读的完整流程。
> **状态：Phase 0 + Phase 1 核心功能已交付。** 剩余尾巴（CBR、适应模式等）转入 Phase 2。
...
- ✅ 打开 CBZ / 图片文件夹
- ✅ 图片渲染（SkiaSharp，支持 JPG/PNG/BMP/GIF/WebP）
- ✅ 缩放（Ctrl+滚轮）+ 平移（中键拖拽）
- ✅ 键盘翻页（← → PgUp PgDn Space）
- ✅ 全屏（F11）
- ✅ 拖放文件打开
- ✅ 错误处理（损坏压缩包、格式不支持等）
- ✅ 加载状态（ProgressBar + 半透明覆盖层）
- ✅ 🎨 演示按钮 → **Debug 模式可见，Release 模式隐藏**（#if !DEBUG）
- ✅ 单元测试 47 个，全部通过
- ✅ 手动端到端测试 9/11 组通过（加载遮罩、压力测试暂搁置）
...
- [x] 实现完整的 `MainWindow.axaml` 布局
  - 顶部工具栏（📂 打开文件 + 📁 打开文件夹 + 🎨 演示按钮「Debug 专属」）
...
- [x] 拖放文件到窗口打开
- [x] 全屏模式（F11）
- [x] 错误处理：文件不存在、格式不支持、损坏压缩包
- [x] 加载状态提示（ProgressBar + 半透明覆盖层）
- [x] 端到端集成测试 → 见 `docs/test-checklist.md`
- [x] 手动测试：多种格式漫画 → 见 `docs/test-checklist.md`
