...
- [x] `LibraryScanner`：扫描监控目录 → 发现 CBZ/CBR/文件夹
- [x] 导入时按优先级提取元数据：ComicInfo.xml > 文件名推测
- [x] 自动提取封面（ComicInfo `FrontCover` 标注 > 第一页）→ 缓存到 `%APPDATA%/NbReader/covers/`
- [x] 计算 `file_hash`（CBZ/CBR: SHA256 前 1MB；文件夹: SHA256(文件名+大小)）
- [x] 新建 `ComicResource` + 关联 Category "未分类"
- [x] 重复文件跳过（`UNIQUE(source_type, source_id)` + hash 辅助判断）

### Day 5：端到端验证 + 测试

- [ ] 手动测试：扫描目录 → 书架展示 → 打开阅读 → 关闭 → 进度恢复（待人工验证）
- [ ] 手动测试：Category 创建/归类/筛选（待人工验证）
- [ ] 手动测试：阅读历史记录与清除（待人工验证）
- [ ] 更新 `test-checklist.md`（待人工验证后勾选）
...
- [x] SQLite 建表正确，`SqliteStorageService` 单元测试全部通过
- [x] 书架能展示本地漫画，封面 + 标题 + 进度条正确
- [x] Category 可创建/重命名/删除/排序，卡片可拖拽归类
- [x] 阅读进度自动保存，重新打开漫画恢复位置
- [x] 阅读历史按日期分组展示，点击可续读
- [x] 筛选（语言/内容类型）正常工作
